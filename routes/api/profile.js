...
// @desc    route for getting User Profiles based on USERNAME
// @access  PUBLIC
...
router.get("/find/everyone", (req, res) => {
  Profile.find()
    ...
    .then(profiles => {
      if (!profiles) {
        res.status(400).json({ usernotfound: "No profile was found" });
      }
      res.json(profiles);
    })
    .catch(error => console.log("Error in fetching Profiles " + error));
});
// @type    DELETE
//@route    /api/profile/
    ...
    Profile.findOneAndRemove({ user: req.user.id })
      .then(() => {
        Person.findOneAndRemove({ _id: req.user.id })
          .then(() => res.json({ success: "Profile Successfully Deleted" }))
...
router.post(
  ...
  (req, res) => {
    Profile.findOne({ user: req.user.id })
      .then(profile => {
        if (!profile) {
          res.status(400).json({ profilenotfound: "user does not exist" });
        }
